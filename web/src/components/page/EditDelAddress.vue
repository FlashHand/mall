<template>
    <div class="edit-wrap">
        ...
        <footer-btn :text="footBnt" :id="$route.params.status" :to="'/address'" :data="data" :prov="prot" :city="city" :district="district" :value="value"></footer-btn>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
            ...
            selectaddress(){
                this.popupVisible = false;
                if(this.prot != undefined && this.city != undefined){
                    this.data.addressAll = this.prot.name +'-'+ this.city.name +'-'+ this.district.name;
                }else{
                    Toast('请选择地址');
                }
            },
            updateCity: function () {
                if(this.addressPlace == '')return;
