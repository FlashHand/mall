...
class GoodsClassModel extends BaseModel implements IsExitsModel
{
   ...
   /* public function select(array $options = array())
    ...
    public function getAandBClass(){
        $one=$this->field(static::$id_d.','.static::$className_d)->where(self::$fid_d.'=0')->select();
        foreach($one as $k=>$v){
            $one[$k]['two']=$this->field(static::$id_d.','.static::$className_d)->where(self::$fid_d.'="%s"',$v['id'])->select();
        }
        return $one;
    }
}